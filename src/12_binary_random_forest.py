...
# Exclude End_Time because it becomes known after the accident ends
...
# 200 trees vote together, which usually improves stability over one Decision Tree
model = Pipeline(
    steps=[
...
).to_csv("reports/binary_random_forest_metrics.csv", index=False)
...
with open("reports/binary_random_forest_classification_report.txt", "w") as file:
    file.write(report)
...
).to_csv("reports/binary_random_forest_confusion_matrix.csv")

joblib.dump(model, "models/binary_random_forest.joblib")
print("--- BINARY RANDOM FOREST COMPLETE ---")